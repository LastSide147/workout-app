...
// Очередь пересчёта рейтинга дня. РАНЬШЕ каждый вызов queueRatingWrite
  // сразу вставал в очередь и выполнялся — десять подходов подряд
  // означали десять отдельных пересчётов (6 записей каждый = 60 лишних
  // записей в Firestore). Теперь пересчёт ОТКЛАДЫВАЕТСЯ и выполняется
  // только один раз — либо когда пользователь уходит с этого экрана
  // (см. useFocusEffect ниже, это основной случай), либо по таймеру
  // подстраховки, если человек остался здесь надолго. Сам ввод
  // повторений (setExerciseEntry чуть выше по коду) НЕ откладывается —
  // он пишется в Firestore сразу же, как и раньше: это первичные данные,
  // терять их при аварийном закрытии приложения нельзя. Откладывается
  // только ПЕРЕСЧЁТ РЕЙТИНГА — производные данные, которые в любой
  // момент можно честно досчитать заново с нуля (см. recalculateAllRatings
  // в WorkoutLogScreen/WorkoutHistoryScreen — он это и делает при
  // каждом заходе на вкладку, это наш "самолечащийся" запасной вариант).
  const ratingQueueRef = useRef(Promise.resolve());
  const ratingDebounceTimerRef = useRef(null);
  const pendingRatingTaskRef = useRef(null);

  // Подстраховка: если пользователь останется на экране очень долго, не
  // переключаясь на другую вкладку, пересчёт всё равно случится сам, не
  // дожидаясь ухода. 5 минут — с большим запасом: обычная пауза между
  // подходами редко больше, а если больше — рейтинг просто досчитается
  // чуть позже, ничего не потеряется и не сломается.
  const BACKUP_DEBOUNCE_MS = 5 * 60 * 1000;

  // Выполняет задачу через ту же последовательную очередь, что была и
  // раньше — это по-прежнему нужно, чтобы два пересчёта не могли
  // выполниться параллельно и не перезаписать результат друг друга.
  const runRatingTask = task => {
    ratingQueueRef.current = ratingQueueRef.current.catch(() => {}).then(task);
    return ratingQueueRef.current;
  };

  // Сбрасывает то, что накопилось (если накопилось) — вызывается и по
  ...
  // Новая версия queueRatingWrite. НЕ выполняет задачу сразу — запоминает
  // её как "последнюю актуальную" (перезаписывая предыдущую, если
  // подходы идут один за другим) и перезапускает таймер подстраховки.
  // Реально задача выполнится только внутри flushPendingRatingWrite.
  const queueRatingWrite = task => {
    pendingRatingTaskRef.current = task;

    if (ratingDebounceTimerRef.current) {
      clearTimeout(ratingDebounceTimerRef.current);
    }
    ratingDebounceTimerRef.current = setTimeout(() => {
      ratingDebounceTimerRef.current = null;
      flushPendingRatingWrite();
    }, BACKUP_DEBOUNCE_MS);
  };

  // Как только уходим с этого экрана — переключились на другую вкладку,
  // либо (в Истории) сменили день, — досчитываем рейтинг СРАЗУ, не
  // дожидаясь пятиминутного таймера. Именно это даёт основную экономию:
  // сколько бы подходов ни было внесено и какие бы паузы ни были между
  // ними (хоть 3 минуты, хоть больше), пока пользователь остаётся на
  // этом экране — лишнего пересчёта не происходит, всё разом досчитывается
  // один раз при уходе.
  useFocusEffect(
    useCallback(() => {
      return () => {
  ...
  const handleRemoveExercise = async exercise => {
    ...
    try {
      ...
      if (remainingNames.length === 0) {
        // Удаление ПОСЛЕДНЕГО упражнения за день (день опустел) — это
        // редкое, осознанное, завершающее действие, а не серия быстрых
        // подряд идущих правок (как обычный ввод повторений) — откладывать
        // его через очередь/задержку ниже смысла нет, а вред есть: раньше
        // это тоже шло через queueRatingWrite, из-за чего старый рейтинг
        // дня (и старые данные по упражнениям) могли оставаться в
        // Firestore намного дольше, чем ожидает пользователь — вплоть до
        // ухода с экрана или пятиминутного таймера-подстраховки. Именно
        // это и создавало "рассинхрон": в приложении день уже пустой, а в
        // базе документ рейтинга ещё старый. Выполняем сразу же, как и
        // applyStatus ниже по файлу (тоже чистит рейтинг дня немедленно).
...
// === Итог: что изменилось и как это работает сейчас ===============
//
// ЧТО БЫЛО. Каждое сохранение подхода запускало recalculateDayRating —
// а это отдельная запись рейтинга дня + обновление 4 бакетов периодов
// (день/неделя/месяц/год) + отметка в профиле = 6 записей в Firestore.
// Плюс сам ввод повторений (setExerciseEntry) — ещё 2 записи. Итого:
// ОДИН подход = 8 записей. Десять подходов подряд = 80 записей, даже
// если между ними были паузы в несколько минут.
//
// ЧТО СТАЛО. Ввод повторений (2 записи) по-прежнему происходит сразу
// на каждый подход — это первичные данные, их нельзя терять. А вот
// пересчёт рейтинга (6 записей) теперь НЕ выполняется на каждый
// подход — он откладывается и выполняется один раз, когда пользователь
// уходит с этого экрана (переключил вкладку, в Истории — сменил день).
// Итог для десяти подходов: 20 записей (ввод) + 6 записей (один
// финальный пересчёт) = 26 записей вместо 80 — экономия больше чем
// втрое, и она НЕ зависит от того, с какими паузами вводились подходы.
// На случай, если пользователь останется на этом экране очень долго,
// не переключаясь никуда, есть таймер-подстраховка на 5 минут — рейтинг
// всё равно досчитается сам.
//
// ПОЧЕМУ ЭТО БЕЗОПАСНО. Если приложение закроют/убьют ДО того, как
// сработает отложенный пересчёт — рейтинг просто останется чуть
// устаревшим до следующего раза. Это не потеря данных: сами повторения
// уже сохранены, а recalculateAllRatings (см. WorkoutLogScreen /
// WorkoutHistoryScreen) при каждом следующем заходе на вкладку
// пересчитывает рейтинг заново с нуля из настоящих записей — то есть
// система сама себя "долечивает" при следующем открытии.
//
// ИСКЛЮЧЕНИЕ ИЗ ОЧЕРЕДИ: удаление ПОСЛЕДНЕГО упражнения за день (день
// опустел) выполняется СРАЗУ ЖЕ, В ОБХОД очереди/задержки выше (см.
// handleRemoveExercise). Изначально это тоже откладывалось наравне с
// обычным пересчётом — из-за этого при очистке дня старый рейтинг
// (и старые данные по упражнениям) мог оставаться в Firestore
// значительно дольше, чем ожидает пользователь, создавая видимость
// рассинхрона данных между приложением и базой (в приложении день уже
// пустой, а документ рейтинга дня в базе — ещё старый). Очистка дня —
// редкое, осознанное, завершающее действие, а не серия быстрых подряд
// идущих правок, поэтому откладывать её ради экономии записей не имеет
// смысла — а вред от задержки в этом конкретном случае был реальным.
//
// ГДЕ САМЫЕ ДОРОГИЕ МЕСТА ОСТАЮТСЯ (не исправлено в этом файле,
// оставлено сознательно на потом):
//   1. fetchLeaderboard (src/services/ratings.js) при открытии
//      Статистики читает ВЕСЬ список пользователей бакета без
//      ограничения (.limit()) — при 1000 активных пользователей это
//      1000 чтений за один открытый экран. Обсуждали и договорились
//      сделать "окно" ±25 записей вокруг самого пользователя вместо
//      всего списка — не реализовано, сделаем отдельно.
//   2. Фильтрация рейтинга по упражнению устроена так, что не может
//      использовать то же "окно" — имена упражнений заданы
//      пользователем и их нельзя проиндексировать заранее (в отличие
//      от возраста/веса/города, у которых конечный, заранее известный
//      список значений). Потребует отдельной перестройки данных
//      (под-документы по каждому упражнению), если понадобится.
//   3. Настоящая защита от злоупотребления (кто-то пишет в Firestore
//      напрямую, в обход приложения) возможна только через Security
//      Rules на стороне сервера — тут её пока нет, ждём, когда
//      пришлёшь текущие правила, чтобы предложить безопасное
//      добавление.